...
def login_form():
    ...
    if st.sidebar.button("Login"):
        ...
        if user in store and verify_password(pw, store[user]["pw_hash"]):
            ...
            # booking defaults (these are reset by clear_on_submit, but kept for first run default)
...
# ------------------------
# Booking Entry (operations & admin) - canonical keys used here
# ------------------------
    ...
    with tabs[1]:
        ...
        with st.form("booking_form", clear_on_submit=True):
            ...
            if not venues:
                ...
                st.selectbox("Select Venue", venues, index=venue_index, key="form_venue")
                ...
                if not courts:
                    ...
                    # Fallback check: Ensure selected court is valid for current venue (Index is crucial)
                    try:
                        court_index = courts.index(st.session_state["form_court"])
                    except (ValueError, KeyError):
                        st.session_state["form_court"] = courts[0]
                        court_index = 0
                        
                    st.selectbox("Select Court / Turf", courts, index=court_index, key="form_court")
            ...
            # compute available slots for selected city/venue/court/date
            ...
            if submitted:
                # validations
                if not form_venue or not form_court:
                    st.error("Please select a valid Venue and Court.")
                elif not booking_name:
...
with tabs[reports_tab_index]:
    ...
    # Safely get current canonical keys
    ...
    if not timeline_df.empty and sel_city and sel_venue and sel_court:
        ...
        st.info("No bookings in system yet or court/venue not selected.")
